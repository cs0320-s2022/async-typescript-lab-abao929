...
sun.addEventListener("change", (event) => postAndUpdate());
moon.addEventListener("change", (event) => postAndUpdate());
rising.addEventListener("change", (event) => postAndUpdate());
...
function postAndUpdate(): void {
  ...
  }).then((response: Response) => response.json()).then((data) => updateSuggestions(data["matches"]));
...
function updateSuggestions(matches: string[]): void {
  ...
  matches.forEach((value) => suggestions.innerHTML += `<li tabindex="0">${value}</li>`);
}
...
document.addEventListener("keyup", async (event : KeyboardEvent) => {
  if (event.key == 'a') {
    await updateValues("Taurus", "Gemini", "Cancer");
    postAndUpdate();
  }
});
